...
use core::fmt::{Display, Formatter, Debug, self};
use core::str::{Chars};
// Because of no_std:
    ...
    pub fn iter(&self) -> RnaIterator<'a> {
        match *self {
            Rna::GivenNucleotides(rna) => {
                RnaIterator::GivenNucleotides(rna.chars())
            }
            Rna::DnaBased(dna) => {
                RnaIterator::DnaBased(dna.chars())
            }
        }
    }
}
...
impl <'a> Iterator for RnaIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            ...
            RnaIterator::GivenNucleotides(chars) => {
                chars.next()
            }
        }
    }
...
/*impl <'a> Display for Rna<'a> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "")
        /*match self {
            Rna::GivenNucleotides(str) => Ok({
                let out = arrform!(300, "Rna {{}}");
                out.as_str()
            }),
            Rna::DnaBased(str) => Ok({
                format!("Rna {{}}")
